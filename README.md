...
| 모델 | 특징 | 적합한 상황 |
|------|------|-------------|
| Opus 4.7 | 1M 컨텍스트, 최고 추론 능력, 높은 단가 | 토요일 — 장시간 심층 작업 (설계, 문서, 대규모 리팩토링) |
| Sonnet 4.6 | 200k 컨텍스트, 빠른 응답, 중간 단가 | 평일 — 단발성 작업 (스크립트, 설정, 간단한 수정) |
| Haiku 4.5 | 200k 컨텍스트, 최고 속도, 낮은 단가 | 일요일 — 가벼운 탐색, 쿨다운 |
...
| 일요일 | Haiku 4.5 (`claude-haiku-4-5`) |
| 한국 공휴일 (ICS 기반) | Opus 4.7 (`claude-opus-4-7`) |
| 그 외 평일 | Sonnet 4.6 (`claude-sonnet-4-6`) |
